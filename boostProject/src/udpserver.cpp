...
void UDPDevice::setSerialParams(const UDPParams &params)
{
	m_serialParams = params;
}

void UDPDevice::setTimeOut(int timeout /*= 3000*/)
{
	m_timeOut = timeout;
}

bool UDPDevice::startThread()
{
	...
	try {
		// 创建线程
		m_thread = boost::thread(boost::bind(&UDPDevice::mainRun, this));
	}
	catch (std::exception &e) {
...
bool UDPDevice::stopThread()
{
	m_pios->stop();
	return true;
}

void UDPDevice::setCallbackFunc(const boost::function<void(DataGramPtr)> &func)
{
	m_dataCallbackFunc = func;
}

bool UDPDevice::writeDataGram(const DataGram &datagram)
{
...
bool UDPDevice::writeRaw(const ByteVector &rawData)
{
...
void UDPDevice::start_a_read()
{
	cout << "SerialPortDevice::start_a_read() " << endl;
	mutex::scoped_lock lock(m_serialMutex);
	m_pSerial->async_read_some(buffer(m_tempBuf), 
		boost::bind(&UDPDevice::readHandler,
		this,
		boost::asio::placeholders::error,
		boost::asio::placeholders::bytes_transferred));
}

void UDPDevice::start_a_write()
{
	if (m_pSerial) {
		cout << "SerialPortDevice::start_a_write() " << endl;
		mutex::scoped_lock lock(m_serialMutex);
		async_write(*m_pSerial, buffer(*(m_writeQueue.front())),
			bind(&UDPDevice::writeHandler, this, boost::asio::placeholders::error));
	}

	
}

void UDPDevice::readHandler(const system::error_code &ec, size_t bytesTransferred)
{
	if (ec) {
	...
	for (size_t i = 0; i < bytesTransferred; i++) {
		...
		case WAITING_FF1: 
		{
			if (byte == (uint8_t)0xFF) {
				...
				m_ptimer->async_wait(bind(&UDPDevice::timeoutHandler,
...
void UDPDevice::writeHandler(const system::error_code &ec)
{
	if (ec) {
...
void UDPDevice::timeoutHandler(const system::error_code &ec)
{
	if (!ec) {
		cout << "Timeout!!" <<endl;
		m_state = WAITING_FF1;
