...
{
	return 0;// arv_buffer_new_full(size, preallocated, NULL, NULL);
}
...
ArvBuffer *ImageBuffer::arv_buffer_new_allocate(size_t size)
{
	return 0;// arv_buffer_new_full(size, NULL, NULL, NULL);
}
...
// 	g_return_val_if_fail(buffer->payload_type == ARV_BUFFER_PAYLOAD_TYPE_CHUNK_DATA ||
// 		buffer->payload_type == ARV_BUFFER_PAYLOAD_TYPE_EXTENDED_CHUNK_DATA, NULL);
...
// 		if (buffer->chunk_endianness == G_BIG_ENDIAN) {
...
const void *ImageBuffer::arv_buffer_get_user_data(ArvBuffer *buffer)
{
	//g_return_val_if_fail(ARV_IS_BUFFER(buffer), NULL);
...
// 		buffer->payload_type == ARV_BUFFER_PAYLOAD_TYPE_EXTENDED_CHUNK_DATA, 0);
...
{
// 	g_return_val_if_fail(ARV_IS_BUFFER(buffer), 0);
// 	g_return_val_if_fail(buffer->payload_type == ARV_BUFFER_PAYLOAD_TYPE_IMAGE ||
// 		buffer->payload_type == ARV_BUFFER_PAYLOAD_TYPE_EXTENDED_CHUNK_DATA, 0);
