	...
	: QWidget(parent)
{
	...
	uint aaa = m_device.GetReg(0xd00);
	uint data = 1000;
	m_device.SetReg(0xd00, data);
	int aa =m_device.GetReg(0xd00);
	std::string strSubNetMask = "192.168.8.123";
	unsigned int nCurrentSubNetMask = inet_addr(strSubNetMask.c_str());
	m_device.SetReg(0xd18, nCurrentSubNetMask);
	unsigned int nc = m_device.GetReg(0xd18);
	int a= 0;
}
