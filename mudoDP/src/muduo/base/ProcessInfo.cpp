...
/*__thread*/ std::vector<int>* t_pids = NULL;
...
int scanDir(const char *dirpath, int (*filter)(const struct dirent *))
{
  ...
  int result = 0;// ::scandir(dirpath, &namelist, filter, alphasort);
...
int g_clockTicks = 0;// static_cast<int>(::sysconf(_SC_CLK_TCK));
int g_pageSize = 0;// static_cast<int>(::sysconf(_SC_PAGE_SIZE));
}  // namespace detail
}  // namespace muduo
...
std::vector<int> ProcessInfo::threads()
{
